...
function assetBuilder(text) {
  ...
  let amount_current_asset, amount_fixed_asset = 0;
  const category_data = text.slice(text.indexOf(category_begin) + category_begin.length).trim();
...
function liabilityBuilder(text) {
  ...
  let amount_current_liability, amount_fixed_liability = 0;
  const category_data = text.slice(text.indexOf(category_begin) + category_begin.length).trim();
  const rows = category_data.split('\n');
  rows.some(function(row_str) {
    let row = row_str.split(' ').filter(r => r !== "");
